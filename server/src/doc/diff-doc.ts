...
import { positiveInt, check } from "../common/validate.js";
import { diffLines, type DiffLine } from "../common/diff.js";
...
export const docDiff: McpTool = {
  ...
  async handler(args) {
    // @validate
    const __v = check(
      positiveInt(args?.version_id_1, "version_id_1"),
      positiveInt(args?.version_id_2, "version_id_2"),
    );
    if (__v) return __v;
