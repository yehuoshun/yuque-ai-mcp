...
export const docVersions: McpTool = {
  ...
  description: "List document version history (last 100 published versions). GET /doc_versions?doc_id=:id. 详见 references/api/doc_api.md",
  ...
  async handler(args) {
    // @validate
    const __v = check(
      positiveInt(args?.doc_id, "doc_id"),
