...
const tools: Tool[] = [
  ...
  },
  {
    ...
    description: "创建关键词索引文档。一个关键词 = 一篇索引文档，标题为关键词。一对多：一个关键词可指向多篇源文档。body 格式：顶部文档标题 + 每个 entry 自带 文档标题/关键词/搜索面/摘要 + entry：{JSON}。自动挂 TOC。",
    inputSchema: {
      type: "object",
      properties: {
        ...
        entries: {
          ...
          description: "源文档指针列表，一个关键词可对应多篇源文档。每项含 did/ns/t/s/url/w 全部必填，可选 ek(entry关键词)/es(entry搜索面)/esum(entry摘要)/et(entry文档标题)",
        },
...
const handlers: Record<string, (args: any) => Promise<string>> = {
  ...
  yuque_config_update: async (a: any) => configUpdate(a),
};
...
async function configStatus(): Promise<string> {
  ...
  if (cfg.route_book_sub.length === 0) {
    ...
    for (const b of cfg.route_book_sub) {
      try {
        ...
        const data = await get(`/repos/${b.book_id}`) as any;
