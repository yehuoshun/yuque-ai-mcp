 ...
 */
export async function diffIndex() {
    ...
    let docMap;
    try {
        const data = await get(`/repos/${indexNamespace}/docs/${DOC_MAP_SLUG}`);
    ...
    for (const entry of docMap) {
        // 2. 知识库级别对比
        try {
            const repoData = await get(`/repos/${entry.book_id}`);
            ...
            if (repoUpdatedAt === entry.last_built)
                continue;
            // 3. 文档级别对比
            const allDocs = await listAllDocs(entry.book_id);
            const changedDocs = allDocs.filter((d) => d.updated_at && d.updated_at > entry.last_built);
