...
export async function kbSearch(params) {
    ...
    const bodies = await Promise.all(uniqueIndexDocs.map(async (doc) => {
        try {
            const data = await get(`/repos/${index_book_id}/docs/${doc.doc_id}`);
    ...
    const dupesRemoved = uniqueIndexDocs.reduce((sum, d) => {
