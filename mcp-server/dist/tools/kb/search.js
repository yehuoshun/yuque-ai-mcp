...
async function findSubIndexesFromAll(tokens, routeBooks, errors) {
    ...
    await Promise.all(routeBooks.map(async (rb) => {
        ...
        for (const p of ptrs) {
            if (!all.has(p.namespace))
                all.set(p.namespace, p);
        }
    }));
...
async function findSubIndexes(tokens, ns, bookId, errors) {
    ...
    await Promise.all(Array.from(seenDocs.keys()).map(async (docId) => {
        try {
            const data = await get(`/repos/${bookId}/docs/${docId}`);
            const body = (data.data || data).body || "";
            const list = Array.isArray(JSON.parse(body)) ? JSON.parse(body) : (JSON.parse(body).e || []);
...
// ─── 翻页搜 ────────────────────────────────────────────
async function fetchAllPages(token, scope, errors) {
    const allHits = [];
    let page = 1;
    let hasMore = true;
    while (hasMore && page <= 5) {
        try {
            const data = await get(`/search?q=${encodeURIComponent(token)}&type=doc&scope=${scope}&page=${page}`);
            const hits = data.data || [];
            for (const r of hits)
                allHits.push({ doc_id: r.id || r.doc_id, title: r.title || "" });
...
// ─── 分批读 body ───────────────────────────────────────
async function batchReadBodies(docs, bookId, errors) {
    const CONCURRENCY = 5;
    const result = [];
...
// ─── 格式化输出 ────────────────────────────────────────
function formatSearchResults(tokens, subIndexes, results, routeErrors) {
