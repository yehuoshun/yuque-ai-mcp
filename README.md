...
|------|--------------------------|--------|
| 用户 | `get_user` | `get_user` + `health_check` 🆕 + `get_user_stats` 🆕 |
| 搜索 | `search` | `search` + `batch_get_docs_body` 🆕 |
| 知识库 | `list_books` `get_book` `create_book` `update_book` | 同左 + `delete_repo` 🆕 + 分组管理 🆕 |
| 文档 | `list_docs` `get_doc` `create_doc` `update_doc` | 同左 + `delete_doc` 🆕 + `list_doc_versions` 🆕 + `get_doc_version` 🆕 |
| 目录 | `get_toc` `update_toc` | 同左 + 目录增强 (clone/cross-book/batch-mount) 🆕 |
...
| **合计** | **16 个** | **47 个**（16 基础 + 31 独有） |

> 🦞 本项目覆盖官方全部 16 个工具，并多出 31 个独有工具：删除/恢复/版本历史/群组/统计/回收站/上传导入/目录增强/个人仪表盘/健康检查/配置管理/分组管理。
...
| `yuque_clone_doc_to_toc` | 文档复制到多个目录位置（多目录支持） |
| `yuque_get_toc_flat` | 获取扁平化目录缓存 |
| `yuque_copy_docs_cross_book` | 跨知识库批量复制文档 |
| `yuque_batch_mount_toc` | 批量创建 TITLE 节点并挂载文档 |
...
| `yuque_batch_get_docs_body` | 批量获取多篇文档 Markdown 正文 |
| `yuque_get_user` | 当前 Token 用户详情 |
| `yuque_get_user_stats` | 个人写作统计仪表盘（全维度，⚠️ 需 Cookie） |
...
| Tool | 说明 |
|------|------|
| `yuque_upload_attachment` | 上传文件到语雀 CDN（image/attachment/video） |
...
│   ├── tsconfig.json
│   └── src/
│       ├── index.ts           # Server 入口（注册 47 个 tools）
