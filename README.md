...
> 语雀全功能 AI Agent 技能 —— 32 MCP Tools + 14 业务 Skills（批量运维/写作辅助/知识分析/翻译/同步），全面超越官方 yuque-ecosystem。纯 LLM + 语雀 API，零外部依赖。
...
```
yuque-mcp (MCP Server)     ← 管理操作：32 个 tools（CRUD、搜索、批量获取、统计、群组、健康检查）
    ↓
业务 Skills                ← 14 个 skill：batch/ 运维 · write/ 写作 · map/ 分析 · 翻译 · 同步
    ↓
...
| `skills/` | Markdown | 业务 Skills（batch/write/map 三分类，14 个技能） |
