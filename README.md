...
> 语雀全功能 AI Agent 技能 —— 35 MCP Tools + 17 业务 Skills（批量运维/写作辅助/知识分析/翻译/同步/导入），全面超越官方 yuque-ecosystem。纯 LLM + 语雀 API，零外部依赖。
...
| 维度 | 🏛 yuque-mcp-server | 🏛 yuque-ecosystem | 🏛 yuque-plugin | 🦞 本项目 |
|------|----------|-----------|-----------|--------|
| 工具总数 | 16 个 | 复用 yuque-mcp-server | 复用 yuque-mcp-server | **35 个** |
...
```
yuque-mcp (MCP Server)     ← 管理操作：35 个 tools（CRUD、搜索、批量获取、导入、统计、群组、健康检查）
    ↓
...
| 组件 | 技术栈 | 说明 |
|------|--------|------|
| `mcp-server/` | TypeScript + `@modelcontextprotocol/sdk` | MCP Server，提供 35 个 tools |
...
基于 MCP 35 tools 的高层业务能力。全部遵循先预览后确认、单篇隔离不传染。
...
│   ├── src/
│   │   ├── index.ts      # Server 入口（注册 35 个 tools）
