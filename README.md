...
| 维度 | 🏛 yuque-mcp-server | 🏛 yuque-ecosystem | 🏛 yuque-plugin | 🦞 本项目 |
|------|----------|-----------|-----------|--------|
| 工具总数 | 16 个 | 复用 yuque-mcp-server | 复用 yuque-mcp-server | **39 个** |
...
```
yuque-mcp (MCP Server)     ← 管理操作：39 个 tools（CRUD、搜索、批量获取、导入、统计、群组、回收站、健康检查）
    ↓
...
基于 MCP 39 tools 的高层业务能力。全部遵循先预览后确认、单篇隔离不传染。
