...
> 语雀全功能 AI Agent 技能 —— 36 MCP Tools + 18 业务 Skills（批量运维/写作辅助/知识分析/翻译/同步/导入），全面超越官方 yuque-ecosystem。纯 LLM + 语雀 API，零外部依赖。
...
| 维度 | 🏛 yuque-mcp-server | 🏛 yuque-ecosystem | 🏛 yuque-plugin | 🦞 本项目 |
|------|----------|-----------|-----------|--------|
| 工具总数 | 16 个 | 复用 yuque-mcp-server | 复用 yuque-mcp-server | **36 个** |
...
```
yuque-mcp (MCP Server)     ← 管理操作：36 个 tools（CRUD、搜索、批量获取、导入、统计、群组、健康检查）
    ↓
业务 Skills                ← 18 个 skill：batch/ 运维 · write/ 写作 · map/ 分析 · 翻译 · 同步 · 导入
    ↓
...
| 组件 | 技术栈 | 说明 |
|------|--------|------|
| `mcp-server/` | TypeScript + `@modelcontextprotocol/sdk` | MCP Server，提供 36 个 tools |
...
| Tool | 说明 |
|------|------|
| `yuque_kb_search` | 知识库管道搜索：token 数组 → N 路并行搜索子索引库 → 去重 → 解析 entries → 返回源文档指针 |
| `yuque_index_create` | 在子索引库创建关键词索引文档（[索引] {keyword}），标准三层格式 + 自动挂 TOC |
...
基于 MCP 36 tools 的高层业务能力。全部遵循先预览后确认、单篇隔离不传染。
...
│       ├── inbox.md       # 碎片收集整理
│       ├── knowledge.md   # 文档关联图谱
│       └── search.md      # 知识库搜索 & 索引构建
├── mcp-server/           # MCP Server (TypeScript)
│   ├── src/
│   │   ├── index.ts      # Server 入口（注册 36 个 tools）
