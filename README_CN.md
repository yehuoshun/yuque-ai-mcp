...
| HTTP 解耦 | ❌ 仅 stdio | ✅ **双模式**：stdio + HTTP SSE（共享注册中心） |
...
| `yuque_update_note` | `PUT /api/v2/notes/:id` | 更新/软删除/恢复。删除需 `confirm='DELETE'` |
...
| `yuque_rss_schedule` | 分析最近更新频率，生成推荐抓取时间并写入配置知识库 |
...
- `kv_slugs`：KV 去重分片文档（单文档 250KB 上限）
...
| `yuque_crawl_schedule` | 分析抓取频率，生成推荐抓取时间 |
