...
基于 MCP 32 tools 的高层业务能力。全部遵循：先预览后确认、单篇隔离不传染、上限 100 篇、结束出报告。
...
| 外部文档导入（本地/Obsidian/Notion ZIP） | [skills/batch/import.md](skills/batch/import.md) |
...
所有管理操作通过 **yuque-mcp** 的 32 个 tools 执行，不需要手动 curl。
...
| Tool | 说明 |
|------|------|
| `yuque_upload_image` | 上传图片到语雀 CDN（需 Cookie 登录态） |
