...
> 仅用于总库路由：命中后挑最相关的 3-5 个路由文档。子库有总库精准定位的 namespace/doc_id，无需筛选。
...
| 阶段 | 并发数 | 说明 |
|------|--------|------|
| 搜索词搜索（总库路由） | 1 | 取并集合成一组，一次搜索 |
| 搜索词搜索（子库） | 1-3 | 多个 namespace 可并发搜 |
