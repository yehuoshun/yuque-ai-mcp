...
以下示例展示未配置索引子库时，从提问到答案的完整路径：

```
用户提问: "Python 协程怎么用"（未配置 index_books）
         │
         ├─[1] LLM 生成 3-4 组搜索关键词（同 4.1 prompt，无变化）
         │      → 并发搜索语雀内容库（scope=default_book.namespace）
         │      → 返回标题 + 摘要列表（含高亮 <em>）
         │
         ├─[2] LLM 标题筛选（同 4.2 prompt）
         │      → 从搜索结果标题中挑 3-5 篇最相关
         │
         ├─[3] 读原文全文
         │      GET /repos/{book_id}/docs/{doc_id}?raw=1
         │      降级模式下没有索引子库提供内容段，直接读全文
         │
         └─[4] LLM 生成答案 + 引用出处（同 4.3 prompt）
                → 原文全文作为上下文传入
```
...
> - 跳过了索引总库路由（无总库可查）
> - 跳过了索引子库搜索（无子库可查）
> - 搜索结果直接来自内容库，没有「内容段→按需读原文」两步，直接读全文
> - 结果精度依赖语雀搜索 API 的分词质量——关键词策略不变，但命中取决于内容库标题/正文匹配度
