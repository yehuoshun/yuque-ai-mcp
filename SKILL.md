...
1. 子索引库：`yuque_list_docs` → 当前文档数 > 200 → 提示用户创建新子索引库
2. 总库：`yuque_list_docs` → 当前路由文档数 > 300 → 提示用户创建新总库
3. 检查结果直接返回给用户，等待确认后才继续构建

### 提示模板

```
⚠️ {库类型}（{库名}）当前已有 {N} 篇文档，接近上限。
建议：
1. 新建一个子索引库（分配新的 book_id 和 namespace）
2. 或者改为用户手动创建并授予权限
