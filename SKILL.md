...
```
yuque-mcp (MCP Server)     ← 管理操作：49 个 tools（CRUD、搜索、导出、导入、统计、群组、回收站、仪表盘、健康检查、配置管理、热重载、分组管理、索引构建与增量更新）
    ↓ 提供 49 个 tools
...
> 📌 **索引架构 v3**（2026-06-05 重构）：单层索引 — 只保留子索引库，砍掉总库路由层。搜索直接并行搜所有子索引库，语雀原生搜索 + 标题语义匹配即可精准命中。详情见 [二、知识库问答系统](#二知识库问答系统)。
...
所有 49 个工具通过 MCP client 自动提供（name + description + inputSchema），Agent 直接调用即可。
...
```
子索引库 (yehuoshun/index-java-1)         ← 关键词索引 + 搜索
...
搜索流程：token 数组 → 并行搜所有子索引库（语雀搜索 API + 标题匹配）→ 命中索引文档 → 读 body 展开 entries → 按 weight 降序返回。
...
正文（Markdown 格式，每个源文档一个块）：

```markdown
## 02丨如何用Canvas绘制层次关系图？

Canvas怎么绘图，Canvas指令式绘图教程，用Canvas画层次关系图，
Canvas2D核心API，Canvas画层次关系图实战

本文介绍Canvas指令式绘图基础，通过绘制层次关系图实战演示Canvas核心API。

- doc_id: 232072822
- 链接: https://www.yuque.com/yehuoshun/huwsx0/kf8s0xue0pfzvl09
- 权重: 6

---

## 28丨Canvas、SVG与WebGL性能对比

Canvas和SVG哪个快，WebGL性能怎么样，Canvas性能优化对比，
渲染性能对比，Canvas vs SVG vs WebGL

本文对比分析Canvas、SVG、WebGL三者在渲染性能上的优劣。

- doc_id: 232303572
- 链接: https://www.yuque.com/yehuoshun/huwsx0/kfxgkov0vt9ux7da
- 权重: 6
...
设计意图：
- **一个文档一块**：每个源文档独立成块，`##` 标题 + 搜索面 + 摘要 + 元数据，语雀全文索引覆盖所有自然语言文本
- **`---` 分隔**：代码层按 `\n---\n` 切分各块，正则提取 `doc_id`、`链接`、`权重`
- **搜索面**：LLM 生成时把关键词、同义词、常见问法、口语表达全部揉进去，最大化命中率
...
| `url` | 元数据行 | 源文档完整链接 |
| `weight` | 元数据行 | 权重 1-10 |
| `search_surface` | 标题和元数据之间的文本 | 搜索面，参与语雀全文索引 |
| `summary` | 搜索面和元数据之间的文本 | 摘要 |
| `tree` | 元数据行（可选） | 章节树 |
