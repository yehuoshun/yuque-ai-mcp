...
索引总库 (yehuoshun/rqgc16)              ← 路由层：只存子索引库指针
    │
    ├── 📄 Java                       → [{book_id, namespace}, ...]
    ├── 📄 Python                     → [{book_id, namespace}, ...]
    └── 📄 前端                       → [{book_id, namespace}, ...]
...
> 语雀知识库有文档数量上限（免费版约 200-300 篇/库）。子索引库和总库逼近上限时，需要新建库或申请扩容。

### 自动策略

LLM 有创建知识库的权限（`yuque_create_repo`），容量不够时直接新建，**不需要通知用户**：

1. 子索引库 > 200 篇文档 → 自动 `yuque_create_repo` 新建 `index-{domain}-{n}`（自增编号）
2. 总库 > 300 篇路由文档 → 自动 `yuque_create_repo` 新建 `index-route-{n}`
3. 新建后自动更新 `yuque-config.json` 的 `route_book` / `route_book_sub`
4. 仅在创建失败（权限不足、达到团队上限）时才通知用户
