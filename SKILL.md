...
```
文档标题：Spring Boot 自动配置原理

关键词：["SpringBoot", "SpringBoot启动", "自动配置", "EnableAutoConfiguration", "条件装配"]

搜索面：SpringBoot怎么自动配置，自动配置是什么，自动配置不生效怎么办，SpringBoot自动配置原理，条件装配怎么用

摘要：SpringBoot 通过 @EnableAutoConfiguration 和 spring.factories 实现自动配置。条件装配注解 @ConditionalOnClass 和 @ConditionalOnMissingBean 按 classpath 动态决定 Bean 注册。
...
| entry | `entry：{JSON 对象}` | ⚠️ 必须且只有 1 个，`{did, ns, t, s, url, w}` 全部必填。一个关键词 = 一篇源文档，一对一精准锚点 |
...
6. 逐关键词写入：
   a. LLM 选择该关键词下最相关的 1 篇 {did, w} → 生成搜索面（keywords[] + search_surface）+ 摘要
...
5. 聚合：所有文档的输出汇总
   ...
   → 按关键词维度分发：每个关键词取最相关的 1 篇 {did, w, t, s}

6. 逐关键词写入子索引库（子代理串行写入，并发=1）：
   a. LLM 选择该关键词下最相关的 1 篇 entry → 生成搜索面 + 摘要
      → 见 §2 的「单关键词写入 Prompt」
...
3. 新增文档 → LLM 读正文 → 判断属于哪些关键词
   → 对每个关键词，创建新关键词索引文档（1 entry = 该文档）
   → 无匹配关键词 → 创建新关键词索引文档
4. 更新文档 → 找到引用它的关键词索引 → 更新索引文档的标题/摘要/搜索面
5. 删除文档 → 找到引用它的关键词索引 → 删除该索引文档（1 entry = 1 源文档）
