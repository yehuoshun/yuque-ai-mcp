...
所有管理操作通过 **yuque-mcp** 的 36 个 tools 执行，不需要手动 curl。
...
| Tool | 说明 |
|------|------|
| `yuque_kb_search` | 管道搜索：token 数组 → N 路并行搜索 → 去重 → 解析 entries → 返回源文档指针 |
| `yuque_index_create` | 创建关键词索引文档（[索引] {keyword}），标准三层格式 + 自动挂 TOC |
