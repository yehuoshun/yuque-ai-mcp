...
```
yuque-mcp (MCP Server)     ← 管理操作：41 个 tools（CRUD、搜索、导出、导入、统计、群组、回收站、仪表盘、健康检查、热重载）
    ↓ 提供 41 个 tools
...
所有 37 个工具通过 MCP client 自动提供（name + description + inputSchema），Agent 直接调用即可。
...
> 语雀知识库文档上限约 5000 篇，远大于索引构建规模，正常情况下不会触发。
> `createIndexDoc` 内置静默检查，仅达到 95% 时才输出警告。
> 若频繁触发，说明索引策略有问题，需排查。
