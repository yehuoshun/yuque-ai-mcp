...
搜索流程：token 数组 → 并行搜所有索引库（语雀搜索 API + 标题匹配）→ 命中索引文档 → 读 body 展开 entries → 按 weight 降序返回。

职责：

| 层 | 内容 | 作用 |
|----|------|------|
| 索引库描述 | JSON `{source_books, last_built}` | 运维数据：覆盖源库、构建时间 |
...
> `source_books`：该索引库覆盖的源知识库 book_id 列表。
