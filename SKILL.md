...
```
yuque-mcp (MCP Server)     ← 管理操作：62 个 tools（CRUD、搜索、导出、导入、统计、群组、回收站、仪表盘、健康检查、配置管理、热重载、分组管理、索引构建与增量更新、目录增强）
    ↓ 提供 62 个 tools
...
基于 MCP 62 tools 的高层业务能力。全部遵循：先预览后确认、单篇隔离不传染、上限 100 篇、结束出报告。
