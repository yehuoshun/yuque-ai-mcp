...
搜索面：SpringBoot怎么自动配置，如何实现多数据源，条件装配是什么，自动配置不生效怎么办，SpringBoot多数据源配置，DataSource读写分离，SpringBoot入门，自动配置原理，条件装配实战
...
| 搜索面 | `搜索面：{纯文本}` | 自然语言搜索面：问句/口语/场景描述，纯文本逗号分隔，放在关键词和摘要之间，提升模糊查询命中率 |
| 摘要 | `摘要：{100-200 字}` | LLM 重排序用，判断相关性 |
| entries | `entries：{JSON 数组}` | 源文档指针，⚠️ 必须且只有 1 个 `[{did, ns, t, s, url, w}]`，**全部必填**。一个关键词 = 一篇源文档，一对一精准锚点 |
