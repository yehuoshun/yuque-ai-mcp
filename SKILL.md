...
正文（JSON 数组，agent 直接 `JSON.parse`）：
...
[
  ...
  {
    "title": "28丨Canvas、SVG与WebGL在性能上的优势与劣势",
    "keywords": ["Canvas", "SVG", "WebGL", "性能对比", "渲染性能"],
    "search_surface": "Canvas和SVG哪个快，WebGL性能怎么样，Canvas性能优化对比",
    "summary": "本文对比分析Canvas、SVG、WebGL三者在渲染性能上的优劣。",
    "doc_id": 232303572,
    "namespace": "yehuoshun/huwsx0",
    "doc_title": "28丨Canvas、SVG与WebGL性能对比.html",
    "slug": "kfxgkov0vt9ux7da",
    "url": "https://www.yuque.com/yehuoshun/huwsx0/kfxgkov0vt9ux7da",
    "weight": 6
  }
]
```
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `title` | ❌ | entry 文档标题，agent 展示用 |
| `keywords` | ❌ | 该 entry 的技术术语数组，`cleanToken` 清洗 |
| `search_surface` | ❌ | 自然语言搜索面，纯文本逗号分隔 |
| `summary` | ❌ | 100-200 字，该 entry 对应源文档的核心内容概括 |
| `doc_id` | ✅ | 源文档 ID |
| `namespace` | ✅ | 源知识库 namespace |
| `doc_title` | ✅ | 源文档标题 |
| `slug` | ✅ | 源文档 slug |
| `url` | ✅ | 源文档完整链接（写入时自动从 namespace+slug 拼接兜底） |
| `weight` | ✅ | 权重 1-10，LLM 判断该文档与关键词的拟合度（越高越相关） |
| `tree` | ❌ | 章节树 `{sections: [{id, title, summary}]}`，文档 > 5000 字时可选 |
