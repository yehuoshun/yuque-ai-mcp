...
| `index_books` | — | 索引子库列表 |
| `route_cache_ttl_days` | 7 | 路由缓存过期天数，0=禁用 |
...
用户提问: "Docker 容器之间怎么通信"
         ...
         │
         ├─[1] 查 route_cache → 命中？
         │      → 命中：直接用缓存的子库列表 + 原始问题搜子库 → 跳到 [4]
         │      → 未命中：继续 [2]
         │
         ├─[2] LLM 生成 3-4 组搜索关键词（空格分隔，覆盖不同表述方式）
         │      → 并发搜索引总库（多组关键词）
         │      → 命中标题匹配的路由文档 → 读全文 → 提取子库 namespace
         │      → 去重（内存 set）→ 写入 route_cache（TTL 可配置，默认 7 天）
         │
...
| 场景 | 耗时 | 说明 |
|------|------|------|
| 首次查询 | ~3s | route_cache miss → LLM 生成搜索词 + 搜总库 + 搜子库 |
| 同类词（缓存期内） | ~0.5s | route_cache 命中 → 原始问题直接搜子库，省一次 LLM 调用（~280 token） |
| 缓存过期 | ~3s | route_cache TTL 过期，重新搜总库 |

## 3. 缓存策略

| 缓存层 | 存储 | TTL | 说明 |
|--------|------|-----|------|
| 路由（关键词→子库） | SQLite | 可配置（默认 7 天） | 低频变更，过期自动重搜 |
| 搜索结果 | SQLite | 10分钟 | 同 query 复用 |
| 索引文档内容 | **不缓存** | — | 每次实时读，保证一致性 |

路由缓存 TTL 通过配置文件 `route_cache_ttl_days` 字段设置（默认 7，设为 0 禁用缓存）。
...
```
正常路径（索引管线）
  ↓ 命中不足 / 未配置索引子库 / route_cache miss 且搜总库无结果
降级模式（跳过索引层，直接搜内容库）
  ↓ 仍 0 命中
返回「未找到相关内容，请尝试换个问法」
```
