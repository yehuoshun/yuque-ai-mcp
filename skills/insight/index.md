...
## 同义词/别名（重要）
对于有多个常见叫法的技术术语，每个叫法都作为独立关键词输出，weight 相同：
- C# → 同时输出 Csharp、C井
- .NET → 同时输出 dotnet
- JavaScript → 同时输出 JS
- TypeScript → 同时输出 TS
- Kubernetes → 同时输出 K8s、K8S
- PostgreSQL → 同时输出 Postgres、PG
- VS Code → 同时输出 VSCode、VisualStudioCode
- WebAssembly → 同时输出 WASM、wasm
- 正则表达式 → 同时输出 正则、Regex、RegExp
- 面向对象 → 同时输出 OOP

⚠️ 只输出社区公认的别名，不要自己编造。不确定就不输出。
⚠️ 每个别名都是独立 keyword，不是 keywords 数组里的子项。
